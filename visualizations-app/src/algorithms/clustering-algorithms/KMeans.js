...
class KMeans extends Component {
  ...
  kmeans = async () => {
    ...
    const randomPoints = this.getRandom(this.props.points, k);
    //create a shallow copy of centroids (to make sure the assigned points don't change)
    let centroids = [...randomPoints];

    //DELETE: Test centroid CSS animation effects on initial centroids
    ...
    do {
      ...
      this.assignToClusters(this.props.points, k, centroids);
  ...
  colorPoint(point) {
    ...
    if (pointElement) {
      pointElement.classList.replace(
        'cluster-unassigned',
        `cluster${point.closestCentroid}`
      );
    }
  }

  moveCentroid(centroid) {}
